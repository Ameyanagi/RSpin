...
pub use processing_2d::{
    ...
    normalize_spectrum_2d_volume_json, offset_spectrum_2d_json, phase_spectrum_2d_json,
    project_spectrum_2d_x_json, project_spectrum_2d_y_json, resample_spectrum_2d_json,
    scale_spectrum_2d_json, shift_spectrum_2d_axes_json, sine_bell_apodization_spectrum_2d_json,
...
/// Parses JCAMP-DX text into serialized `Spectrum2D` JSON.
///
/// # Errors
///
/// Returns an error when parsing or serialization fails.
pub fn parse_jcamp_dx_2d_json(input: &str) -> Result<String> {
...
///
/// Returns an error when parsing or serialization fails.
pub fn parse_jeol_jdf_1d_bytes_json(input: &[u8]) -> Result<String> {
    let spectrum = read_jeol_jdf_1d_bytes(input)?;
    spectrum1d_to_json(&spectrum)
}

/// Parses JEOL Delta `.jdf` bytes into serialized `Spectrum2D` JSON.
///
/// # Errors
///
/// Returns an error when parsing or serialization fails.
pub fn parse_jeol_jdf_2d_bytes_json(input: &[u8]) -> Result<String> {
    let spectrum = read_jeol_jdf_2d_bytes(input)?;
    spectrum2d_to_json(&spectrum)
}
...
pub fn spectrum_bundle_1d_by_source_format_json(input: &str, format: &str) -> Result<String> {
    let bundle = read_spectrum_bundle_json(input)?;
...
/// Returns an error unless the bundle contains exactly one matching
/// one-dimensional spectrum, or when serialization fails.
pub fn spectrum_bundle_1d_by_source_vendor_json(input: &str, vendor: &str) -> Result<String> {
    let bundle = read_spectrum_bundle_json(input)?;
...
/// Returns an error unless the bundle contains exactly one matching
/// one-dimensional spectrum, or when serialization fails.
pub fn spectrum_bundle_1d_by_source_path_json(input: &str, source_path: &str) -> Result<String> {
    let bundle = read_spectrum_bundle_json(input)?;
