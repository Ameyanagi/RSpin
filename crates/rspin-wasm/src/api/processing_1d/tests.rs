...
use super::super::{spectrum1d_from_json, to_json};
use super::*;
...
fn offsets_shifts_and_zero_fills_1d_spectrum_json() -> anyhow::Result<()> {
    ...
    let processed = spectrum1d_from_json(&filled_json)?;
...
fn crops_1d_spectrum_json() -> anyhow::Result<()> {
    ...
    let cropped = spectrum1d_from_json(&cropped_json)?;
...
fn takes_absolute_value_1d_spectrum_json() -> anyhow::Result<()> {
    ...
    let processed = spectrum1d_from_json(&abs_json)?;
...
fn resamples_1d_spectrum_json() -> anyhow::Result<()> {
    ...
    let resampled = spectrum1d_from_json(&resampled_json)?;
...
fn roundtrips_1d_fft_json() -> anyhow::Result<()> {
    ...
    let inverse = spectrum1d_from_json(&inverse_json)?;
...
fn phases_and_magnitudes_1d_spectrum_json() -> anyhow::Result<()> {
    ...
    let phased = spectrum1d_from_json(&phased_json)?;
    ...
    let magnitude = spectrum1d_from_json(&magnitude_json)?;
...
fn apodizes_and_subtracts_baseline_json() -> anyhow::Result<()> {
    ...
    let apodized = spectrum1d_from_json(&apodized_json)?;
    ...
    let gaussian = spectrum1d_from_json(&gaussian_json)?;
    ...
    let sine_bell = spectrum1d_from_json(&sine_bell_json)?;
    ...
    let corrected = spectrum1d_from_json(&corrected_json)?;
