...
use super::super::{
    align_spectra_by_peak_to_matrix_1d_json, align_spectra_by_zone_to_matrix_2d_json, from_json,
    generate_spectrum_matrix_1d_json, generate_spectrum_matrix_2d_json, to_json,
};
...
#[test]
fn aligns_spectra_by_zone_to_matrix_2d_json() -> anyhow::Result<()> {
    let spectra_json = to_json(&vec![
        Spectrum2D::new(
            Axis::linear("x", Unit::Ppm, 0.0, 2.0, 3)?,
        ...
        Spectrum2D::new(
            Axis::linear("x", Unit::Ppm, 0.5, 2.5, 3)?,
            Axis::linear("y", Unit::Ppm, -0.25, 1.75, 3)?,
            vec![0.0, 0.0, 0.0, 0.0, 7.0, 0.0, 0.0, 0.0, 0.0],
            Metadata::named("shifted"),
        )?,
    ])?;
    let result_json = align_spectra_by_zone_to_matrix_2d_json(
        &spectra_json,
        "{}",
        r#"{"target_x_axis":null,"target_y_axis":null,"outside_value":0.0}"#,
    )?;
    let result: rspin_analysis::ZoneAlignedMatrix2D = from_json(&result_json)?;
