...
pub fn load_spectra_1d_strict(path: impl AsRef<Path>) -> Result<SpectrumBundle> {
    SpectrumBundleLoader::new().strict().read_bundle_1d(path)
}

/// Loads all two-dimensional spectra from a file or directory path as a bundle in strict mode.
///
/// # Errors
///
/// Returns an error when loading fails, any candidate fails to read, or no
...
/// Returns an error when loading fails, any candidate fails to read, or no
/// one-dimensional bundle data is found.
pub fn load_spectra_1d_strict_relative_to(
    base: impl AsRef<Path>,
    path: impl AsRef<Path>,
) -> Result<SpectrumBundle> {
    SpectrumBundleLoader::new()
        .strict()
        .read_bundle_1d_relative_to(base, path)
}

/// Loads all two-dimensional spectra from one selected path in strict mode relative to a base directory.
///
/// # Errors
///
/// Returns an error when loading fails, any candidate fails to read, or no
/// two-dimensional bundle data is found.
pub fn load_spectra_2d_strict_relative_to(
    base: impl AsRef<Path>,
    path: impl AsRef<Path>,
) -> Result<SpectrumBundle> {
    SpectrumBundleLoader::new()
...
pub fn load_spectra_1d_many_strict<I, P>(paths: I) -> Result<SpectrumBundle>
where
    I: IntoIterator<Item = P>,
    P: AsRef<Path>,
{
    SpectrumBundleLoader::new()
        .strict()
        .read_bundle_1d_many(paths)
}

/// Loads all two-dimensional spectra from multiple selected paths as one bundle in strict mode.
///
/// # Errors
///
/// Returns an error when loading fails, any selected path or candidate fails to
/// read, or no two-dimensional bundle data is found.
pub fn load_spectra_2d_many_strict<I, P>(paths: I) -> Result<SpectrumBundle>
where
    I: IntoIterator<Item = P>,
    P: AsRef<Path>,
{
...
/// Returns an error when loading fails, any selected path or candidate fails to
/// read, or no one-dimensional bundle data is found.
pub fn load_spectra_1d_many_strict_relative_to<I, P>(
    base: impl AsRef<Path>,
    paths: I,
) -> Result<SpectrumBundle>
where
    ...
    P: AsRef<Path>,
{
    SpectrumBundleLoader::new()
        .strict()
        .read_bundle_1d_many_relative_to(base, paths)
}

/// Loads all two-dimensional spectra from selected paths in strict mode relative to a base directory.
///
/// # Errors
///
/// Returns an error when loading fails, any selected path or candidate fails to
/// read, or no two-dimensional bundle data is found.
pub fn load_spectra_2d_many_strict_relative_to<I, P>(
    base: impl AsRef<Path>,
    paths: I,
) -> Result<SpectrumBundle>
where
