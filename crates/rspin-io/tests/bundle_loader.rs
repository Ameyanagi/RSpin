...
#[test]
fn reader_first_short_source_aliases_load_directly() -> anyhow::Result<()> {
    let reader = RSpinReader::new();
    let root = nmrxiv_fixture_root();

    let first = reader.read_first_by_vendor(&root, "bruker")?;
    assert_eq!(first.source().vendor(), Some(LoadedSourceVendor::Bruker));

    let jcamp = reader.read_first_1d_by_format(&root, "jdx")?;
    assert!(jcamp.metadata.nucleus.is_some());

    let (with_source, source) = reader.read_first_1d_with_source_by_format(&root, "jcamp")?;
    assert!(with_source.metadata.nucleus.is_some());
    assert_eq!(source.format(), "jcamp_dx");

    let hsqc = reader.read_first_2d_by_vendor(&root, "jeol")?;
    assert_eq!(hsqc.shape(), (1024, 32));

    let raw = reader.read_first_by_data_kind(&root, LoadedSourceDataKind::Raw)?;
    assert_eq!(raw.source().data_kind(), LoadedSourceDataKind::Raw);

    let base = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("testdata/nmrxiv/cc0");
    let first_many = reader.read_first_many_by_vendor_relative_to(&base, ["myrcene"], "jeol")?;
    assert_eq!(first_many.source().vendor(), Some(LoadedSourceVendor::Jeol));
    Ok(())
}

#[test]
fn free_first_short_source_aliases_load_directly() -> anyhow::Result<()> {
    let root = nmrxiv_fixture_root();

    let first = io::load_first_spectrum_by_vendor(&root, "jeol")?;
    assert_eq!(first.source().vendor(), Some(LoadedSourceVendor::Jeol));

    let jcamp = io::load_first_spectrum_1d_by_format(&root, "jcamp dx")?;
    assert!(jcamp.metadata.nucleus.is_some());

    let (jcamp_with_source, jcamp_source) =
        io::load_first_spectrum_1d_with_source_by_format(&root, "jdx")?;
    assert!(jcamp_with_source.metadata.nucleus.is_some());
    assert_eq!(jcamp_source.format(), "jcamp_dx");

    let hsqc = io::load_first_spectrum_2d_by_vendor(&root, "jeol")?;
    assert_eq!(hsqc.shape(), (1024, 32));

    let base = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("testdata/nmrxiv/cc0");
    let (bruker_2d, bruker_source) =
        io::load_first_spectrum_2d_many_with_source_by_data_kind_relative_to(
            &base,
            ["myrcene/bruker_cosy_raw"],
            LoadedSourceDataKind::Raw,
        )?;
...
#[test]
fn reader_first_source_strict_helpers_load_directly() -> anyhow::Result<()> {
    let reader = RSpinReader::new();
    let base = fixture_root();

    let first = reader.read_first_spectrum_strict_by_source_relative_to(
        &base,
        "varian_1h",
        LoadedSourceFilter::agilent(),
    )?;
    assert!(first.is_1d());
    assert_eq!(first.source().path(), Some(Path::new("varian_1h")));

    let proton = reader.read_first_1d_strict_by_source_relative_to(
        &base,
        "varian_1h",
    ...
    let (_, source) = reader.read_first_1d_with_source_strict_by_sources_relative_to(
        &base,
        "varian_1h",
        [LoadedSourceFilter::agilent()],
    )?;
    ...
    let mixed = nmrxiv_fixture_root();
    let hsqc = reader.read_first_2d_strict_by_source_relative_to(
        &mixed,
        "jeol",
    ...
    let first_many = reader.read_first_spectrum_many_strict_by_source_relative_to(
        &base,
        ["empty_jcamp/empty.jdx", "varian_1h"],
        LoadedSourceFilter::agilent(),
    )?;
...
#[test]
fn free_first_source_strict_helpers_load_directly_and_reject_selected_bad_source()
-> anyhow::Result<()> {
    let base = fixture_root();

    let first = io::load_first_spectrum_strict_by_source_relative_to(
        &base,
        "varian_1h",
        LoadedSourceFilter::agilent(),
    )?;
    ...
    let (proton, source) = io::load_first_spectrum_1d_with_source_strict_by_source_relative_to(
        &base,
        "varian_1h",
        LoadedSourceFilter::raw(),
    )?;
    ...
    let mixed = nmrxiv_fixture_root();
    let hsqc = io::load_first_spectrum_2d_strict_by_sources_relative_to(
        &mixed,
        "jeol",
        [LoadedSourceFilter::jeol()],
    )?;
    ...
    let selected_bad = io::load_first_spectrum_1d_many_strict_by_source_relative_to(
        &base,
        ["empty_jcamp/empty.jdx", "varian_1h"],
        LoadedSourceFilter::jcamp(),
    );
    let Err(error) = selected_bad else {
        anyhow::bail!("strict source-filtered loading should reject a selected malformed source");
    };
    assert!(matches!(error, RSpinError::Parse { .. }));
    Ok(())
}
...
#[test]
fn reader_first_typed_source_strict_helpers_load_directly() -> anyhow::Result<()> {
    let reader = RSpinReader::new();
    let base = fixture_root();

    let first = reader.read_first_spectrum_strict_by_source_vendor_relative_to(
        &base,
        "varian_1h",
        "varian",
    )?;
    ...
    let proton = reader.read_first_1d_strict_by_source_data_kind_relative_to(
        &base,
        "varian_1h",
        LoadedSourceDataKind::Raw,
    )?;
    ...
    let (_, source) = reader.read_first_1d_with_source_strict_by_source_format_relative_to(
        &base,
        "varian_1h",
        "agilent fid",
    )?;
    ...
    let mixed = nmrxiv_fixture_root();
    let hsqc = reader.read_first_2d_strict_by_source_path_relative_to(
        &mixed,
        "jeol",
    ...
    let first_many = reader.read_first_spectrum_many_strict_by_source_path_prefix_relative_to(
        &base,
        ["empty_jcamp/empty.jdx", "varian_1h"],
        "varian_1h",
    )?;
...
#[test]
fn free_first_typed_source_strict_helpers_load_and_reject_selected_bad_source() -> anyhow::Result<()>
{
    let base = fixture_root();

    let first = io::load_first_spectrum_strict_by_source_format_relative_to(
        &base,
        "varian_1h",
        "agilent fid",
    )?;
    assert!(first.is_1d());
    assert_eq!(first.source().format(), "agilent_fid");

    let (proton, source) =
        io::load_first_spectrum_1d_with_source_strict_by_source_data_kind_relative_to(
            &base,
            "varian_1h",
            LoadedSourceDataKind::Raw,
        )?;
    ...
    let mixed = nmrxiv_fixture_root();
    let hsqc = io::load_first_spectrum_2d_strict_by_source_path_prefix_relative_to(
        &mixed, "jeol", "jeol",
    )?;
    assert_eq!(hsqc.shape(), (1024, 32));

    let selected_bad = io::load_first_spectrum_1d_many_strict_by_source_path_relative_to(
        &base,
        ["empty_jcamp/empty.jdx", "varian_1h"],
        "empty_jcamp/empty.jdx",
    );
    let Err(error) = selected_bad else {
        anyhow::bail!("strict typed source loading should reject a selected malformed source");
    };
    assert!(matches!(error, RSpinError::Parse { .. }));
    Ok(())
}
...
#[test]
fn reader_short_exact_source_aliases_load_directly() -> anyhow::Result<()> {
    let reader = RSpinReader::new();
    let base = fixture_root();

    let proton = reader.read_1d_by_vendor_relative_to(&base, "varian_1h", "varian")?;
    assert_eq!(proton.metadata.nucleus, Some(Nucleus::Hydrogen1));

    let raw =
        reader.read_1d_by_data_kind_relative_to(&base, "varian_1h", LoadedSourceDataKind::Raw)?;
    assert_eq!(raw.x.unit, Unit::Seconds);

    let (_, source) =
        reader.read_1d_with_source_by_format_relative_to(&base, "varian_1h", "agilent fid")?;
    assert_eq!(source.format(), "agilent_fid");

    let mixed = nmrxiv_fixture_root();
    let hsqc = reader.read_2d_by_path_prefix_relative_to(&mixed, "jeol", "jeol")?;
    assert_eq!(hsqc.shape(), (1024, 32));

    let (hsqc, hsqc_source) =
        reader.read_2d_many_with_source_by_vendor_relative_to(&mixed, ["jcamp", "jeol"], "jeol")?;
    assert_eq!(hsqc.shape(), (1024, 32));
    assert_eq!(
        hsqc_source.path(),
        Some(Path::new("jeol/myrcene_hsqc_400mhz.jdf"))
...
#[test]
fn free_short_strict_exact_source_aliases_reject_selected_bad_source() -> anyhow::Result<()> {
    let base = fixture_root();

    let proton = io::load_spectrum_1d_many_strict_by_path_prefix_relative_to(
        &base,
        ["empty_jcamp/empty.jdx", "varian_1h"],
        "varian_1h",
    )?;
    ...
    let (_, source) = io::load_spectrum_1d_with_source_strict_by_format_relative_to(
        &base,
        "varian_1h",
        "agilent fid",
    )?;
    ...
    let raw = io::load_spectrum_1d_strict_by_data_kind_relative_to(
        &base,
        "varian_1h",
        LoadedSourceDataKind::Raw,
    )?;
    ...
    let mixed = nmrxiv_fixture_root();
    let (hsqc, hsqc_source) =
        io::load_spectrum_2d_with_source_strict_by_vendor_relative_to(&mixed, "jeol", "jeol")?;
    assert_eq!(hsqc.shape(), (1024, 32));
    assert_eq!(hsqc_source.vendor(), Some(LoadedSourceVendor::Jeol));

    let selected_bad = io::load_spectrum_1d_many_strict_by_format_relative_to(
        &base,
        ["empty_jcamp/empty.jdx", "varian_1h"],
        "jdx",
    );
    let Err(error) = selected_bad else {
        anyhow::bail!("strict short exact loading should reject a selected malformed source");
    };
    assert!(matches!(error, RSpinError::Parse { .. }));
    Ok(())
}
